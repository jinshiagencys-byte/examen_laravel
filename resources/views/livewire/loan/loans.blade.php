            ...
            <x-table>
                <x-slot name="head">
                    <x-table.row>
                        ...
                        <x-table.heading sortable wire:click="sortBy('user_full_name')" :direction="$sorts['user_full_name'] ?? null" class="col-2">{{ __('ui.user') }}</x-table.heading>
                        <x-table.heading sortable wire:click="sortBy('status_id')" :direction="$sorts['status_id'] ?? null" class="col-1">{{ __('ui.status') }}</x-table.heading>
                        ...
                        <x-table.heading sortable wire:click="sortBy('end_date_time')" :direction="$sorts['end_date_time'] ?? null" class="col-1">{{ __('ui.end_date') }}</x-table.heading>
                        <x-table.heading sortable wire:click="sortBy('details')" :direction="$sorts['details'] ?? null" class="col-2">{{ __('ui.details') }}</x-table.heading>
                        <x-table.heading class="col-2">{{ __('ui.assets') }}</x-table.heading>
                        ...
                        <x-table.row wire:key="row-{{ $loan->id }}">
                            ...
                            <x-table.cell class="col">
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    @if($loan->status_id == 1)
                                        <x-button.success wire:click="book({{ $loan->id }})" ><x-loading wire:target="book({{ $loan->id }})" />Valider l'emprunt</x-button.success>
                                        <x-button.danger wire:click="cancel({{ $loan->id }})" ><x-loading wire:target="cancel({{ $loan->id }})" />{{ __('ui.cancel') }}</x-button.danger>
                                    @else
                                        <x-button.success wire:click="complete({{ $loan->id }})" ><x-loading wire:target="complete({{ $loan->id }})" />Terminer</x-button.success>
                                    @endif
                                    <x-button.primary wire:click="edit({{ $loan->id }})"><x-loading wire:target="edit({{ $loan->id }})" />{{ __('ui.edit') }}</x-button.primary>
                                </div>
                            </x-table.cell>
                        </x-table.row>
                    @empty
